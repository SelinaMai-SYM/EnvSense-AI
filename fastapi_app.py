...
from utils.io_utils import repo_root, resolve_feedback_output_root, resolve_mode_data_paths
from utils.time_utils import now_iso


Mode = Literal["study", "sleep"]
Source = Literal["realtime", "offline"]
SleepFeedback = Literal["slept_well", "okay", "poor_sleep"]
...
class SleepFeedbackIn(BaseModel):
    feedback: SleepFeedback
    # Optional grouping for multiple feedback entries in the same session.
...
def _resolve_csv(*, mode: Mode, source: Source) -> Path:
    ...
    mode_paths = resolve_mode_data_paths(mode)
    if mode_paths.sensor_csv is None:
        raise FileNotFoundError(f"no offline CSV available for mode={mode}")
    return mode_paths.sensor_csv
...
def create_app() -> FastAPI:
    ...
    def api_view(
        ...
        source: Source = Query("realtime", description="realtime | offline"),
    ) -> Any:
    ...
    @app.post("/api/sleep/feedback")
    def api_sleep_feedback(payload: SleepFeedbackIn = Body(...)) -> JSONResponse:
        feedback_root = resolve_feedback_output_root()
        ...
        session_dir = feedback_root / session_name
        session_dir.mkdir(parents=True, exist_ok=True)

        fp = session_dir / "feedback.csv"
        ...
        try:
            with fp.open("a", newline="", encoding="utf-8") as f:
                ...
                writer.writerow([now_iso(), payload.feedback])
        except Exception as e:
