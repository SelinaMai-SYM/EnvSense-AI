...
def render_sleep_guard_page(*, csv_path: str | Path) -> None:
    st.markdown("## Sleep Guard (Sleep Mode)")
    ...
    st.markdown("### Sleep Check-in (placeholder)")
    st.caption("Future expansion: collect feedback to improve sleep-readiness model.")

    if "sleep_feedback" not in st.session_state:
        st.session_state.sleep_feedback = None

    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("Slept well", use_container_width=True):
            st.session_state.sleep_feedback = "slept_well"
    with c2:
    ...
    with c3:
        if st.button("Poor sleep", use_container_width=True):
            st.session_state.sleep_feedback = "poor_sleep"

    if st.session_state.sleep_feedback:
        st.success(f"Recorded (not persisted yet): {st.session_state.sleep_feedback}")
