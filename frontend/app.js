...
async function submitSleepFeedback(feedback) {
  state.sleepFeedback = feedback;
  updateUIForMode();
  try {
    const res = await fetch("/api/sleep/feedback", {
      method: "POST",
    ...
    el("checkin-status").textContent = `Saved: ${data.feedback} (${data.session_name})`;
  } catch (err) {
    console.error(err);
...
function wireEvents() {
  ...
  el("btn-checkin-slept").addEventListener("click", () => submitSleepFeedback("slept_well"));
  el("btn-checkin-okay").addEventListener("click", () => submitSleepFeedback("okay"));
  el("btn-checkin-poor").addEventListener("click", () => submitSleepFeedback("poor_sleep"));
}
